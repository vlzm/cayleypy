...
class BeamSearchAlgorithm:
    """Beam Search algorithm for CayleyGraph."""
    ...
    def run(
        ...
        *,
        ...
        start_states = self.graph.state_ops.encode_states(start_state)
        ...
        if self.graph.central_state_hash[0] == layer1_hashes[0]:
            # Start state is the central state.
            return BeamSearchResult(True, 0, [], debug_scores, self.graph.definition)

        for i in range(max_iterations):
            # Create states on the next layer.
            layer2, layer2_hashes = self.graph.graph_utils._get_unique_states(self.graph.get_neighbors(layer1))

            if bool(isin_via_searchsorted(self.graph.central_state_hash, layer2_hashes)):
                # Path found.
                path = None
                if return_path:
                    path = self.graph.path_finder._restore_path(all_layers_hashes, self.graph.central_state)
                return BeamSearchResult(True, i + 1, path, debug_scores, self.graph.definition)

            # Pick `beam_width` states with lowest scores.
            if len(layer2) >= beam_width:
                scores = predictor(self.graph.state_ops.decode_states(layer2))
        ...
        # Path not found.
        return BeamSearchResult(False, 0, None, debug_scores, self.graph.definition)

    def initialize_states(self, list_generators, device):
    ...
    def get_unique_states(self, states: torch.Tensor, vec_hasher = 'Auto',device = 'Auto') -> torch.Tensor:
        '''
    ...
    def beam_search_torch(self, n_beam_search_steps_back_to_ban, n_steps_limit, beam_width, beam_search_models_or_heuristics, batch_size, verbose, state_start, state_destination, list_generators, model, device, dtype):
        ...
        if n_beam_search_steps_back_to_ban > 0:
            hash_initial_state = torch.sum(state_start.view(-1, len(list_generators[0])) * vec_hasher, dim=1)
            vec_hashes_current = hash_initial_state.expand(beam_width * n_generators, n_beam_search_steps_back_to_ban).clone()
        ...
        for i_step in range(1, n_steps_limit + 1):
            ...
            t1 = time.time()
            array_new_states = self.get_unique_states(array_new_states)
            ...
            if n_beam_search_steps_back_to_ban > 0:
                ...
                if mask_new_sum > 0:
                    ...
                    break
                i_cyclic_index_for_hash_storage = (i_cyclic_index_for_hash_storage + 1) % n_beam_search_steps_back_to_ban
            ...
            if array_new_states.shape[0] > beam_width:
                ...
                elif beam_search_models_or_heuristics == 'model_with_predict':
    ...
    def beam_search_path(
        ...
        for i_step in range(1, CFG['n_steps_limit'] + 1):
            ...
            if not CFG.get('ban_p0_p1_transposition_if_p0_lt_p1', False):
                neighbors = self.graph.get_neighbors(array_beam_states)
