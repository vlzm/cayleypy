...
class CayleyGraph:
    ...
    def encode_states(self, states: Union[torch.Tensor, np.ndarray, list]) -> torch.Tensor:
        ...
        if self.string_encoder is not None:
            a = self.string_encoder.encode(states)
            return a
        return states
    ...
    # def get_neighbors_hash(self, states: torch.Tensor) -> torch.Tensor:
    ...
    def _random_walks_bfs_hash(
        ...
        x = [start_state]
        y = [torch.full((1,), 0, device=self.device, dtype=torch.int64)]
        ...
        for i_step in range(1, length):
            ...
            y.append(torch.full((layer_size,), i_step, device=self.device, dtype=torch.int64))
